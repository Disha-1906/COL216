...
int main(int argc, char *argv[])
{
    ...
    while(stall==true || stall79 == false || (default_state.IF_1_Stage.nop==0) || (default_state.IF_2_Stage.nop==0) || (default_state.ID_1_Stage.nop==0)  || (default_state.ID_2_Stage.nop==0) || (default_state.RR_Stage.nop==0) || (default_state.EX_Stage.nop==0) || (default_state.MEM_1_Stage.nop==0) || (default_state.MEM_2_Stage.nop==0) || (default_state.WB_Stage.nop==0)){
...
//----------------------------------------------RR Stage----------------------------------------------------------------------
    ...
    if(default_state.RR_Stage.nop==0){
            ...
            new_state.EX_Stage.read_1 = default_state.RR_Stage.read_1;
            new_state.EX_Stage.read_2 = default_state.RR_Stage.read_2;
    ...
    if(stall== true && ((cycle == stall_marker_1+2) || (cycle == stall_marker_2+1) || (cycle == stall_marker_3 + 4) )){
        ...
        if(default_state.ID_2_Stage.nop==0){
            ...
            if(compare(ins,"add") || compare(ins,"sub" ) || compare(ins,"and") || compare(ins,"or")  || compare(ins,"slt") || compare(ins,"mul")){
                ...
                new_state.EX_Stage.read_1 = mips->registers[mips->registerMap[default_state.ID_2_Stage.Instr[2]]];
                new_state.EX_Stage.read_2 = mips->registers[mips->registerMap[default_state.ID_2_Stage.Instr[3]]];
...
//---------------------------------------------------------------------------------------------------------------
            ...
            if(default_state.RR_Stage.nop == 0 && default_state.ID_2_Stage.nop == 0){
                if(new_state.EX_Stage.format == 2 && (new_state.RR_Stage.format == 1)){
                    if((new_state.EX_Stage.rt == new_state.RR_Stage.rs) || (new_state.EX_Stage.rt == new_state.RR_Stage.rt)){
                        new_state.RR_Stage.nop = 1;
                        stall_marker_3 = cycle;
                        stall_marker_79 = -1;
                        stall=true; 
                    }
                }
                if(new_state.EX_Stage.format == 2 && (new_state.RR_Stage.format == 5)){
                    if((new_state.EX_Stage.rt == new_state.RR_Stage.rs)){
                        new_state.RR_Stage.nop = 1;
                        stall_marker_3 = cycle;
                        stall_marker_79 = -1;
                        stall=true; 
                ...
                if(0==1){
                if((new_state.EX_Stage.rd == new_state.RR_Stage.rs) || (new_state.EX_Stage.rd == new_state.RR_Stage.rt)){
                    new_state.RR_Stage.nop = 1;
                    stall_marker_1 = cycle;
                    stall=true;
                }
                }
                if(new_state.EX_Stage.format == 5 && ( new_state.RR_Stage.format == 1 || new_state.RR_Stage.format == 5 ||
                   new_state.RR_Stage.format == 4 || new_state.RR_Stage.format == 7)){
                    if((new_state.EX_Stage.rt == new_state.RR_Stage.rs) || (new_state.EX_Stage.rt == new_state.RR_Stage.rt)){
                        new_state.RR_Stage.nop = 1;
                        stall_marker_1 = cycle;
                ...
                if(new_state.EX_Stage.format == 5 && (new_state.RR_Stage.format == 2 )){
                    if((new_state.EX_Stage.rt == new_state.RR_Stage.rs)){
                        new_state.RR_Stage.nop =1;
                        stall_marker_1 = cycle;
                        stall =true;
                ...
                if(new_state.EX_Stage.format == 5 && (new_state.RR_Stage.format == 3 )){
                    if((new_state.EX_Stage.rt == new_state.RR_Stage.rs)){
                        new_state.RR_Stage.nop =1;
                        stall_marker_1 = cycle;
                        stall =true;
                ...
                if(stall==false){
                    if(default_state.EX_Stage.nop==0 && default_state.ID_2_Stage.nop == 0){
                    if((new_state.WB_Stage.dest_r == new_state.RR_Stage.rs) || (new_state.WB_Stage.dest_r == new_state.RR_Stage.rt)){
                        new_state.RR_Stage.nop = 1;
                        stall =true;
...
//--------------------------------------------------------------IF_1 Stage----------------------------------------------------
    if(default_state.IF_1_Stage.PC >= mips->commands.size()){
            default_state.IF_1_Stage.nop = 1;
            if(set_complete == false){completed = cycle-1; set_complete = true;}
        }
        ...
        if(default_state.IF_1_Stage.nop==0){
            ...
            if(stall == true){
                ...
                new_state.ID_1_Stage.Instr = default_state.ID_1_Stage.Instr;
                new_state.ID_2_Stage.Instr = default_state.ID_2_Stage.Instr;
            }
            else{
