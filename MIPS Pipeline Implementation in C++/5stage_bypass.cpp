...
struct MIPS_Architecture
{
    ...
    void parseCommand(std::string line)
	{
		...
		if (command.size()>0){
		    // std::cerr << "commands contains something\n";
		    // cout << command.size() << endl;
		    // cout << commands.size() << endl;
		}
		
// 		std::cerr << "unable to know\n";
	};
    void constructCommands(std::ifstream &file)
	{
		...
		file.close();
// 		cout << commands ;
		// std::cerr << "completed commands construction\n";
	};
};
...
int main(int argc, char *argv[])
{
    ...
    default_state.IF_Stage.nop=false;
    ...
    default_state.ID_Stage.nop=true;
    ...
    default_state.EX_Stage.nop=true;
    ...
    default_state.MEM_Stage.nop=true;
    ...
    bool jump = false;
    // cout << "Hello" << endl;
    // while(( !default_state.IF_Stage.nop || !default_state.ID_Stage.nop || !default_state.EX_Stage.nop || !default_state.MEM_Stage.nop || !default_state.WB_Stage.nop)){
    while(true){
        mips->printRegisters(cycle);
        // ------------------WB Stage --------------------------------
        if(!default_state.WB_Stage.nop){
            if(default_state.WB_Stage.Reg_Read){
                int write_data;
                if(!default_state.WB_Stage.MemtoReg){
                ...
                mips->registers[mips->registerMap[default_state.WB_Stage.dest_r]] =  write_data;
                // cout << "I result is " << write_data << endl;
                // cout << "dest_r is " << default_state.WB_Stage.dest_r << endl; 
                // cout << "in reg is " << mips->registers[mips->registerMap[default_state.WB_Stage.dest_r]] << endl;
                // cout << " reg is " << mips->registerMap[default_state.WB_Stage.dest_r] << endl;
                }
        ...
        if(default_state.MEM_Stage.nop){ cout << "0" << endl; }
        if(!default_state.MEM_Stage.nop){
            if(default_state.MEM_Stage.Mem_Write){
                mips->data[default_state.MEM_Stage.ALU_result]=default_state.MEM_Stage.store_data;
                // cout << "store_data is " << default_state.MEM_Stage.store_data << endl;
                // cout << "alu result in MEM  " << default_state.MEM_Stage.ALU_result << endl;
                cout << "1 " << default_state.MEM_Stage.ALU_result << " " << mips->data[default_state.MEM_Stage.ALU_result] << endl;
                // cout << " " << endl;
            }
            else if(default_state.MEM_Stage.Mem_Read){
                if(default_state.MEM_Stage.MemtoReg){
                    if(default_state.MEM_Stage.format == 2){
                    new_state.WB_Stage.I_result = mips->data[default_state.MEM_Stage.ALU_result];
                    // cout << "alu is " << default_state.MEM_Stage.ALU_result << endl;
                    // cout << "data is " << mips->data[default_state.MEM_Stage.ALU_result] << endl; 
                    // cout << "data[1025] is " << mips->data[1025] << endl; 
                    }
                }
                cout << "0" << endl;
                // cout << " " << endl;
...
//mem-ex forwarding inserted here
        if(!default_state.EX_Stage.nop){
            if(default_state.WB_Stage.Reg_Read == true && default_state.WB_Stage.dest_r != "$zero" && default_state.WB_Stage.dest_r == default_state.EX_Stage.rs){
                if(default_state.WB_Stage.format==2 && default_state.EX_Stage.format==1){
                    default_state.EX_Stage.read_1 =  default_state.WB_Stage.I_result;
                    // cout << "read1 is " << new_state.EX_Stage.read_1 << endl;
                }
                else if(default_state.WB_Stage.format == 2 && default_state.EX_Stage.format == 5){
                    default_state.EX_Stage.read_1 =  default_state.WB_Stage.I_result;
                }
            }
            if(default_state.WB_Stage.Reg_Read == true && default_state.WB_Stage.dest_r != "$zero" && default_state.WB_Stage.dest_r == default_state.EX_Stage.rt){
                if(default_state.WB_Stage.format==2 && default_state.EX_Stage.format==1){
                    default_state.EX_Stage.read_2 =  default_state.WB_Stage.I_result;
                    // cout << "read2 is " << default_state.EX_Stage.read_2 << endl;
                }
            }
            ...
            if(default_state.EX_Stage.format == 1){
                // cout << "read 1 is " << default_state.EX_Stage.read_1 << endl;
                // cout << "read 2 is " << default_state.EX_Stage.read_2 << endl;
            }
            ...
            // cout << "format is " << default_state.EX_Stage.format << endl;
            // cout << "result is " << result;
            int e = default_state.EX_Stage.ALU_op;

            new_state.MEM_Stage.ALU_result = result;
            // cout << "result forwarded is " << new_state.MEM_Stage.ALU_result << endl;
            ...
            if(default_state.EX_Stage.RegDst){new_state.MEM_Stage.dest_r = default_state.EX_Stage.rd;}
            ...
            // Add followed by store DM DM forwarding
            if(new_state.WB_Stage.Reg_Read == true && new_state.WB_Stage.dest_r != "$zero" && new_state.WB_Stage.dest_r == default_state.EX_Stage.rs){
                if(new_state.WB_Stage.format == 1 && new_state.MEM_Stage.format == 3){
                    new_state.MEM_Stage.store_data=new_state.WB_Stage.R_result;
                }
                else if(new_state.WB_Stage.format == 5 && new_state.MEM_Stage.format == 3){
                    new_state.MEM_Stage.store_data=new_state.WB_Stage.I_result;
                }
                else if(new_state.WB_Stage.format== 5 && new_state.MEM_Stage.format ==3){
                    new_state.MEM_Stage.ALU_result=new_state.WB_Stage.I_result;
                }
                else if(new_state.WB_Stage.format == 2 && new_state.MEM_Stage.format == 3){
                    new_state.MEM_Stage.store_data=new_state.WB_Stage.I_result;
                }
            }
           // mem to ex
            // if(new_state.WB_Stage.Reg_Read == true && new_state.WB_Stage.dest_r != "$zero" && new_state.WB_Stage.dest_r == new_state.EX_Stage.rs){
            //     if(new_state.WB_Stage.format==2 && new_state.EX_Stage.format==1){
            //         new_state.EX_Stage.read_1 =  new_state.WB_Stage.R_result;
            //         cout << "read1 is " << new_state.EX_Stage.read_1 << endl;
            //     }
            //     else if(new_state.WB_Stage.format == 2 && new_state.EX_Stage.format == 5){
            //         new_state.EX_Stage.read_1 =  new_state.WB_Stage.I_result;
            //     }
            // }
            // if(new_state.WB_Stage.Reg_Read == true && new_state.WB_Stage.dest_r != "$zero" && new_state.WB_Stage.dest_r == new_state.EX_Stage.rt){
            //     if(new_state.WB_Stage.format==2 && new_state.EX_Stage.format==1){
            //         new_state.EX_Stage.read_2 =  new_state.WB_Stage.R_result;
            //         cout << "read2 is " << new_state.EX_Stage.read_2 << endl;
            //     }
            // }
        }
        // ------------------------ID Stage---------------------------------------
        if(stall== true && (cycle == stall_marker+1)){
                stall=false;
                forward_1 = true;
                default_state.ID_Stage.nop=0;
            }
        new_state.EX_Stage.nop = default_state.ID_Stage.nop;
        if(!default_state.ID_Stage.nop){
            ...
            if(forward_1 == true){
                if(new_state.MEM_Stage.dest_r == new_state.EX_Stage.rs){
                    new_state.EX_Stage.read_1=new_state.WB_Stage.I_result;
                }
                else if(new_state.MEM_Stage.dest_r == new_state.EX_Stage.rt){
...
//---------------------------------------------------------------FORWARDING CODE---------------------------------------------------------
            ...
            int data_write;
            if(new_state.WB_Stage.RegDst == true){
                data_write = new_state.WB_Stage.R_result;
            }         
            else{
                data_write= new_state.WB_Stage.I_result;
            }
            if(default_state.EX_Stage.nop == 0 ){
            if(new_state.MEM_Stage.Reg_Read == true && new_state.MEM_Stage.dest_r != "$zero" && new_state.MEM_Stage.dest_r == new_state.EX_Stage.rs){
                //EX-EX FORWARDING
                if((new_state.MEM_Stage.format == 1 || new_state.MEM_Stage.format == 5)  && (new_state.EX_Stage.format==1 || new_state.EX_Stage.format==5 || new_state.EX_Stage.format==4 ||  new_state.EX_Stage.format==7  )){
                    // cout << "I am in " << new_state.MEM_Stage.ALU_result << endl;
                    new_state.EX_Stage.read_1 = new_state.MEM_Stage.ALU_result;
                }
                //MEM-EX FORWARDING WITH ONE STALL
                if(new_state.MEM_Stage.format == 2 && (new_state.EX_Stage.format==1 || new_state.EX_Stage.format==5 || new_state.EX_Stage.format==4 || new_state.EX_Stage.format==7 )){
                    stall =true;
                    // cout << "in forwarding 1" << endl;
                    stall_marker = cycle;
                    new_state.EX_Stage.nop = 1;
                }
                else if((new_state.MEM_Stage.format == 1 || new_state.MEM_Stage.format == 5) && (new_state.EX_Stage.format == 2)){
                    new_state.EX_Stage.read_1 = new_state.MEM_Stage.ALU_result;
                    // cout << "ALU_result is " << new_state.MEM_Stage.ALU_result << endl;
                    // cout << "In forwarding " << endl;
                }
            }  

            

            if(new_state.MEM_Stage.Reg_Read == true && new_state.MEM_Stage.dest_r != "$zero" && new_state.MEM_Stage.dest_r == new_state.EX_Stage.rt){
                if((new_state.MEM_Stage.format == 1 || new_state.MEM_Stage.format == 5) && (new_state.EX_Stage.format==1 || new_state.EX_Stage.format==4 || new_state.EX_Stage.format==7 )){
                new_state.EX_Stage.read_2 = new_state.MEM_Stage.ALU_result;
                }
                else if(new_state.MEM_Stage.format == 2 && (new_state.EX_Stage.format==1 || new_state.EX_Stage.format==4 || new_state.EX_Stage.format==7 )){
                    stall =true;
                    // cout << "in forwarding 2" << endl;
                    stall_marker = cycle;
                    new_state.EX_Stage.nop = 1;
                }
                else if((new_state.MEM_Stage.format == 1 || new_state.MEM_Stage.format == 5) && (new_state.EX_Stage.format == 3)){
                    // cout << "In sw forwarding " << endl;
                    new_state.EX_Stage.read_2 = new_state.MEM_Stage.ALU_result;
                    // cout << "Alu result is " <<  new_state.EX_Stage.read_2 << endl;
                }
            }
            }
//IMPORTANT NOTE : Remenber that the condition given in the book has been modified fo the below two if statements ....
// the third statement in the not bracket has been modified from (new_state.MEM_Stage.dest_r!=rs) to (new_state.MEM_Stage.dest_r==rs)
            // cout << "dest_r is " << new_state.WB_Stage.dest_r << endl;
            // cout << new_state.WB_Stage.format << endl;
            // cout << default_state.ID_Stage.Instr[0] << endl;
            // cout << "rs is " << new_state.EX_Stage.rs << endl;
            if(new_state.WB_Stage.Reg_Read == true && new_state.WB_Stage.dest_r!="$zero" && (new_state.WB_Stage.dest_r==
            new_state.EX_Stage.rs)){
                // cout << "in 1-3-1" << endl;
                new_state.EX_Stage.read_1 = data_write; //create data_write from I result and R result
            }
            if(new_state.WB_Stage.Reg_Read == true && new_state.WB_Stage.dest_r!="$zero"  && (new_state.WB_Stage.dest_r==
            new_state.EX_Stage.rt)){
                // cout << "in 1-3-2 " << endl;
                new_state.EX_Stage.read_2 = data_write; //create data_write from I result and R result
            }
...
//-----------------------------------------------------------------------------------------------------------------------------------------            
        ...
        if(default_state.IF_Stage.PC >= mips->commands.size()){
            default_state.IF_Stage.nop = true;
            bool c = default_state.IF_Stage.nop;
        }
        bool d = default_state.IF_Stage.nop;
        new_state.ID_Stage.nop = default_state.IF_Stage.nop;
        if(!default_state.IF_Stage.nop){
            ...
            else if(!default_state.IF_Stage.PCSrc){
